...
return App_table::find('tasks')
    ->outputUsing(function ($params) {
        ...
        // Soft delete: exclude deleted tasks if column exists
        if ($this->ci->db->field_exists('deleted', db_prefix() . 'tasks')) {
            $where[] = 'AND deleted = 0';
        }
        ...
        $result = data_tables_init(
            ...
            [
                ...
                '(SELECT CONCAT(firstname, " ", lastname) FROM ' . db_prefix() . 'staff WHERE staffid = ' . db_prefix() . 'tasks.addedfrom) as created_by',
                db_prefix() . 'tasks.addedfrom as created_by_id',
            ]
        );
        ...
        foreach ($rResult as $aRow) {
            ...
            $outputName .= '<a href="' . admin_url('tasks/view/' . $aRow['id']) . '" class="main-tasks-table-href-name tw-truncate tw-max-w-xs tw-block tw-min-w-0 tw-font-medium' . (!empty($aRow['rel_id']) ? ' mbot5' : '') . '" title="'.e($aRow['task_name']).'">' . e($aRow['task_name']) . '</a>';
            ...
            if ($aRow['rel_name']) {
                ...
                if ($aRow['rel_type'] === 'ticket') {
                    $relName = sprintf('%s %s #%s', _l('task_related_to'), _l('ticket'), $aRow['rel_id']);
                }
        ...
        <a href="#" class="' . $class . ' tasks-table-start-timer" onclick="timer_action(this,' . $aRow['id'] . '); return false;">' . _l('task_start_timer') . '</a>
            ...
            $row[] = '<img src="' . staff_profile_image_url($aRow['created_by_id']) . '" class="staff-profile-image-xs mright5" /> ' . e($aRow['created_by']);

            $relatedTo = 'General Task';
            if (!empty($aRow['rel_type'])) {
                $relatedTo = _l($aRow['rel_type']);
            }
            $row[] = e($relatedTo);
