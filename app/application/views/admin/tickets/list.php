                    ...
                    <div class="tw-block md:tw-hidden">
                        <?php $this->load->view('admin/tickets/summary', [
                            'hrefAttrs' => function ($status) use ($table) {
                                if ($table && ($rule = $table->findRule('status'))) {
                                    return '@click.prevent="extra.ticketsRules = ' . app\services\utilities\Js::from($rule->setValue([(int) $status['ticketstatusid']])) . '"';
                                } else {
                                    return '';
                                }
                            },
                            'handlersHrefAttr' => function () use ($table) {
                                if ($table && ($rule = $table->findRule('ticket_handlers'))) {
                                    return '@click.prevent="extra.ticketsRules = ' . app\services\utilities\Js::from($rule->setValue('1')) . '"';
                                } else {
                                    return '';
                                }
                            },
                            'ownerHrefAttr' => function () use ($table) {
                                if ($table && ($rule = $table->findRule('has_owner'))) {
                                    return '@click.prevent="extra.ticketsRules = ' . app\services\utilities\Js::from($rule->setValue('1')) . '"';
                                } else {
                                    return '';
                                }
                        ...
                        <div class="md:tw-flex md:tw-items-center">
                            ...
                            <div class="ltr:tw-ml-auto rtl:tw-mr-auto">
                                <app-filters
                                    id="<?= $table->id(); ?>"
                                    view="<?= $table->viewName(); ?>"
                                    :rules="extra.ticketsRules || <?= app\services\utilities\Js::from($chosen_ticket_status && $table && ($rule = $table->findRule('status')) ? $rule->setValue([(int) $chosen_ticket_status]) : []); ?>"
