...
<div class="horizontal-scrollable-tabs panel-full-width-tabs">
    ...
    <div class="horizontal-tabs">
        <ul class="nav nav-tabs nav-tabs-horizontal" role="tablist">
            ...
            <?php /* Email-to-ticket (piping) settings tab hidden temporarily */ ?>
...
<div class="tab-content mtop15">
    <div role="tabpanel" class="tab-pane active" id="set_tickets_general">
        ...
        <hr />
        <?php render_yes_no_option('enable_predefined_replies', 'enable_predefined_replies'); ?>
        <hr />
        <?php render_yes_no_option('enable_knowledge_base_links', 'enable_knowledge_base_links'); ?>
        <hr />
        <?php
...
echo render_select('settings[default_ticket_reply_status]', $statuses, ['ticketstatusid', 'name'], 'default_ticket_reply_status', get_option('default_ticket_reply_status'), [], [], '', '', false); ?>
        <hr />
        <?= render_input('settings[ticket_reassign_pending_expiry_years]', 'settings_ticket_reassign_pending_expiry_years', get_option('ticket_reassign_pending_expiry_years'), 'number', ['min' => 0, 'step' => 1]); ?>
        <p class="text-muted"><?= _l('settings_ticket_reassign_pending_expiry_years_hint'); ?></p>
        <hr />
        <?= render_input('settings[ticket_close_pending_expiry_hours]', 'settings_ticket_close_pending_expiry_hours', get_option('ticket_close_pending_expiry_hours'), 'number', ['min' => 0, 'step' => 1]); ?>
        <p class="text-muted"><?= _l('settings_ticket_close_pending_expiry_hours_hint'); ?></p>
        <hr />
    ...
    <?php /* Email-to-ticket (piping) settings pane hidden temporarily */ ?>
