                            ...
                            <li>
                                <a href="#"
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="mr-2">
                                            <img src="<?= staff_profile_image_url($_staff['staffid']); ?>" class="rounded-circle" width="24" height="24" alt="Profile">
                                        </div>
                                        <div>
                                            <div class="font-weight-bold"><?= e($_staff['firstname'] . ' ' . $_staff['lastname']); ?></div>
                                            <small class="text-muted">
                                                <?php
                                                // Get employee code from custom fields
                                                $emp_code = '';
                                                $division = '';
                                                $this->db->select('cfv.value, cf.slug');
                                                $this->db->from(db_prefix() . 'customfieldsvalues cfv');
                                                $this->db->join(db_prefix() . 'customfields cf', 'cf.id = cfv.fieldid');
                                                $this->db->where('cfv.relid', $_staff['staffid']);
                                                $this->db->where('cfv.fieldto', 'staff');
                                                $this->db->where_in('cf.slug', ['staff_emp_code', 'staff_division']);
                                                $custom_fields = $this->db->get()->result_array();

                                                foreach ($custom_fields as $field) {
                                                    if ($field['slug'] == 'staff_emp_code') {
                                                        $emp_code = $field['value'];
                                                    } elseif ($field['slug'] == 'staff_division') {
                                                        $division = $field['value'];
                                                    }
                                                }

                                                echo $emp_code;
                                                if ($emp_code && $division) echo ' - ';
                                                echo $division;
                                                ?>
                                            </small>
                                        </div>
