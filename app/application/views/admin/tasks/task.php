...
<style>
    #_task_modal .approval-rel-loading {
        margin-top: 6px;
        font-size: 12px;
        color: #64748b;
        display: none;
    ...
    #_task_modal .approval-rel-loading:not(.hide) {
        display: inline-flex;
    }
    #_task_modal .approval-rel-select-group {
        display: block;
    }
    #_task_modal .approval-rel-dropdown-add {
        font-weight: 600;
        color: #16a34a;
        display: block;
        padding: 6px 12px;
            ...
            <div class="modal-body">
                <div class="row">
                  ...
                  ?>
                        ...
                        $value = isset($task) ? $task->name : $prefillTaskName;
                        ?>
                      ...
                      $value = _d(date('Y-m-d'));
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <?php
                     $availableMemberIds = array_map(static function ($member) {
                         return isset($member['staffid']) ? (int) $member['staffid'] : 0;
                     }, $members ?? []);
                     $defaultFollowers = [];
                     if (!empty($prefill_ticket_followers) && is_array($prefill_ticket_followers)) {
                         $defaultFollowers = array_map('intval', $prefill_ticket_followers);
                     } elseif (get_option('new_task_auto_follower_current_member') == '1') {
                         $defaultFollowers = [get_staff_user_id()];
                     }
                     $defaultFollowers = array_values(array_intersect($defaultFollowers, $availableMemberIds));
                     $selectedFollowers = !empty($defaultFollowers) ? $defaultFollowers : '';
                     echo render_select('followers[]', $members, ['staffid', ['firstname', 'lastname']], 'task_single_followers', $selectedFollowers, ['multiple' => true], [], '', '', false);
                     ?>
                            </div>
                        </div>
                        <?php } ?>
               ...
               $descriptionValue = isset($task) ? $task->description : ($prefill_task_description ?? '');
               echo render_textarea('description', '', $descriptionValue, ['rows' => 6, 'placeholder' => _l('task_add_description'), 'data-task-ae-editor' => true, !is_mobile() ? 'onclick' : 'onfocus' => (!isset($task) || isset($task) && $task->description == '' ? 'init_editor(\'.tinymce-task\', {height:200, auto_focus: true});' : '')], [], 'no-mbot', 'tinymce-task'); ?>
