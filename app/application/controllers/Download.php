...
class Download extends App_Controller
{
...
} elseif ($folder_indicator == 'sla_attachment') {
    // SLA attachments download - only for staff members
    if (! is_staff_logged_in()) {
        show_404();
    }

    $this->db->where('id', $attachmentid);
    $attachment = $this->db->get(db_prefix() . 'sla_attachments')->row();

    if (! $attachment) {
        show_404();
    }

    // Get ticket ID from URL parameter or from SLA entry
    $ticketId = $this->input->get('ticket_id');
    if (!$ticketId) {
        // Fallback: get from SLA entry
        $this->db->select('ticket_id');
        $this->db->from(db_prefix() . 'sla_entries');
        $this->db->where('id', $attachment->sla_entry_id);
        $entry = $this->db->get()->row();
        $ticketId = $entry ? $entry->ticket_id : null;
    ...
    $path = get_upload_path_by_type('ticket_sla_attachments') . $ticketId . '/' . $attachment->file_name;
        } else {
