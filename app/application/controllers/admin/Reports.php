...
class Reports extends AdminController
{
    ...
    private $ci;
    /**
     * Cached report access selections for the current staff member.
     * @var array
     */
    private $report_access = [];
    ...
    public function __construct()
    {
        ...
        // Preload report access selections for current staff
        $meta = get_staff_meta(get_staff_user_id(), 'report_access');
        $this->report_access = is_string($meta) ? json_decode($meta, true) : [];
        if (!is_array($this->report_access)) {
            $this->report_access = [];
        }
    }
    ...
    {
        $this->ensure_report_access_allowed('knowledge-base-reports');
    ...
    {
        $this->ensure_report_access_allowed('leads-reports');
    ...
    {
        $this->ensure_report_access_allowed('sales-reports');
    ...
    {
        $this->ensure_report_access_allowed('expenses-reports');
    ...
    {
        $this->ensure_report_access_allowed('expenses-vs-income-reports');
    ...
    public function tickets_lifecycle()
    {
        $this->ensure_report_access_allowed('tickets-lifecycle-reports');

        $this->load->model('tickets_model');
        $this->load->model('departments_model');
        $this->load->model('divisions_model');

        $data['title'] = _l('ticket_lifecycle_report');

        // Get filters from POST/GET
        $from_date = $this->input->post('from_date') ? to_sql_date($this->input->post('from_date')) : null;
        $to_date = $this->input->post('to_date') ? to_sql_date($this->input->post('to_date')) : null;
        $department_filter = $this->input->post('department') ?: [];
        $division_filter   = $this->input->post('division') ?: [];
        $status_filter     = $this->input->post('status') ?: [];

        $department_filter = is_array($department_filter) ? array_filter($department_filter) : array_filter([$department_filter]);
        $division_filter   = is_array($division_filter) ? array_filter($division_filter) : array_filter([$division_filter]);
        $status_filter     = is_array($status_filter) ? array_filter($status_filter) : array_filter([$status_filter]);
        ...
        if ($this->input->is_ajax_request()) {
            $tickets = $this->reports_model->get_ticket_lifecycle_report($from_date, $to_date, $department_filter, $status_filter, $division_filter);
            ...
            foreach ($tickets as $ticket) {
                $row = [
                    '<a href="' . admin_url('tickets/ticket/' . $ticket['ticketid']) . '" target="_blank">#' . $ticket['ticket_number'] . '</a>',
                    '<a href="' . admin_url('tickets/ticket/' . $ticket['ticketid']) . '" target="_blank">' . $ticket['subject'] . '</a>',
                    $ticket['created_date_formatted'],
                    $ticket['created_by_firstname'] . ' ' . $ticket['created_by_lastname'],
                    $ticket['assigned_to_firstname'] && $ticket['assigned_to_lastname'] ?
                        $ticket['assigned_to_firstname'] . ' ' . $ticket['assigned_to_lastname'] : 'Unassigned',
                    $ticket['handlers_list'] ?: 'No handlers',
                    $ticket['department_name'] ?: 'No department',
                    $ticket['service_name'] ?: 'No service',
                    $ticket['priority_name'] ?: 'No priority',
                    $ticket['status_name'] ?: 'Unknown',
                    $ticket['first_staff_reply_date_formatted'] ?: 'No reply',
                    $ticket['response_time_hours'] ? $ticket['response_time_hours'] . 'h' : 'N/A',
                    $ticket['fixed_date_formatted'] ?: 'Not fixed',
                    $ticket['closed_date_formatted'] ?: 'Open',
                    $ticket['lifespan_hours'] ? $ticket['lifespan_hours'] . 'h' : 'N/A',
                    $ticket['reopen_count'] > 0 ? 'Yes (' . $ticket['reopen_count'] . ')' : 'No'
                ];
                $output['data'][] = $row;
            }
        ...
        $data['tickets'] = $this->reports_model->get_ticket_lifecycle_report($from_date, $to_date, $department_filter, $status_filter, $division_filter);
        $data['selected_departments'] = $department_filter;
        $data['selected_divisions']   = $division_filter;
        $data['selected_statuses']    = $status_filter;
        $data['can_export_report']    = $this->report_access_can_export('tickets-lifecycle-reports');

        $this->load->view('admin/reports/tickets_lifecycle', $data);
    }

    /**
     * Enforce per-report access configured on staff profile.
     *
     * @param string $slug Report access key saved in user_meta
     */
    private function ensure_report_access_allowed($slug)
    {
        ...
        $config = $this->report_access[$slug] ?? [];
    ...
    /**
     * Check if current staff can export a specific report.
     */
    private function report_access_can_export($slug)
    {
        if (is_admin()) {
            return true;
        }
        $config = $this->report_access[$slug] ?? [];
        return !empty($config['export']);
    }
}
