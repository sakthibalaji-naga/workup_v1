...
class Dashboard extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['team_ticket_report'] = (new app\services\TicketsReportByStaffTeam())->filterBy('this_month');
        $data['team_ticket_creation_report'] = (new app\services\TicketsReportByStaffTeamCreation())->filterBy('this_month');

        // Check for welcome popup database flag
        $this->db->select('welcome_popup_shown');
        $this->db->where('staffid', get_staff_user_id());
        $staff = $this->db->get(db_prefix() . 'staff')->row();
        $data['show_welcome_popup'] = ($staff && $staff->welcome_popup_shown == 0);
        log_message('debug', 'Dashboard: welcome_popup_shown database flag check: ' . ($data['show_welcome_popup'] ? 'true' : 'false'));
    ...
    public function team_ticket_widget($type)
    {
        $data['team_ticket_report'] = (new app\services\TicketsReportByStaffTeam())->filterBy($type);
        $this->load->view('admin/dashboard/widgets/team_ticket_report_table', $data);
    }

    public function team_ticket_creation_widget($type)
    {
        $data['team_ticket_creation_report'] = (new app\services\TicketsReportByStaffTeamCreation())->filterBy($type);
        $this->load->view('admin/dashboard/widgets/team_ticket_creation_report_table', $data);
    }

    public function group_members_ticket_widget($type)
    {
        $data['team_ticket_report'] = (new app\services\TicketsReportForGroupMembers())->filterBy($type);
        $this->load->view('admin/dashboard/widgets/group_members_ticket_report_table', $data);
    }
}
