...
class Authentication_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            // If email not found and it's staff login, try employee code
            if (!$user && $staff == true && !empty($email)) {
                $this->db->select(db_prefix().'staff.*, cfv.value as employee_code');
                $this->db->from(db_prefix().'staff');
                $this->db->join(db_prefix().'customfieldsvalues cfv', 'cfv.relid = '.db_prefix().'staff.staffid AND cfv.fieldto = "staff" AND cfv.fieldid IN (SELECT id FROM '.db_prefix().'customfields WHERE slug = "staff_emp_code")', 'left');
                $this->db->where('cfv.value', $email);
                $this->db->limit(1);
                $user = $this->db->get()->row();
            }

            if ($user) {
                // Login credential is valid, check password
                if (!$user->password || !app_hasher()->CheckPassword($password, $user->password)) {
                    ...
                    $loginIdentifier = isset($user->employee_code) ? $user->employee_code : $email;
                    log_activity('Failed Login Attempt [Identifier: ' . $loginIdentifier . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
                ...
                log_activity('Non Existing User Tried to Login [Email/Code: ' . $email . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // Password reset is successful, send notification email (don't fail on email error)
            try {
                if ($staff == false) {
                    send_mail_template('customer_contact_password_resetted', $user->email, $user->userid, $user->id);
                } else {
                    send_mail_template('staff_password_resetted', $user->email, $userid);
                }
            } catch (\Throwable $e) {
                log_message('error', 'Password reset notification dispatch failed: ' . $e->getMessage());
            }

            // Always return true after successful password update
            return true;
        }
    ...
    /**
     * Forgot password by employee code
     * @param string $emp_code
     * @return array|boolean
     */
    public function forgot_password_by_empcode($emp_code)
    {
        // Find staff by employee code from custom fields
        $this->db->select(db_prefix() . 'staff.*, cfv.value as employee_code');
        $this->db->from(db_prefix() . 'staff');
        $this->db->join(db_prefix() . 'customfieldsvalues cfv', 'cfv.relid = ' . db_prefix() . 'staff.staffid AND cfv.fieldto = "staff" AND cfv.fieldid IN (SELECT id FROM ' . db_prefix() . 'customfields WHERE slug = "staff_emp_code")', 'left');
        ...
        if ($user) {
            log_activity('Password Reset Initiated [Employee Code: ' . $emp_code . ', Staff ID: ' . $user->staffid . ', IP: ' . $this->input->ip_address() . ']');

            return ['user' => $user];
        }

        log_activity('Non Existing Employee Code Tried Password Reset [Employee Code: ' . $emp_code . ', IP: ' . $this->input->ip_address() . ']');
     ...
     * @return boolean
     */
    public function verify_otp_code($staffid, $otp)
    {
        $session_otp = $this->session->userdata('password_reset_otp_' . $staffid);
        $otp_time    = $this->session->userdata('password_reset_otp_time_' . $staffid);

        if (!$session_otp || !$otp_time) {
            return false;
        }

        // Get configurable OTP expiry time (in minutes), default to 10 minutes
        $otp_expiry_minutes = get_option('otp_expiry_minutes', 10);
        $otp_expiry_seconds = $otp_expiry_minutes * 60;

        // Check if OTP is expired
        if (time() - $otp_time > $otp_expiry_seconds) {
            return false;
        }

        if ($session_otp == $otp) {
            // Check if this OTP has already been used (additional security check)
            $otp_used = $this->db->where('staffid', $staffid)
                                ->where('otp_code', $otp)
                                ->where('otp_used', 1)
                                ->get('tbl_sms_password_reset_logs')
                                ->num_rows() > 0;
            ...
            $this->session->unset_userdata('password_reset_otp_' . $staffid);
            $this->session->unset_userdata('password_reset_otp_time_' . $staffid);
            $this->session->unset_userdata('password_reset_otp_email_' . $staffid);
     ...
     * @param string $otp
     */
    private function send_otp_by_sms($phone, $otp)
    {
        if (!empty($phone)) {
            $this->load->library('sms/App_sms');
            $this->app_sms->trigger(SMS_TRIGGER_STAFF_PASSWORD_RESET, $phone, ['otp_code' => $otp]);
        }
    }
