...
class Tasks_model extends App_Model
{
    ...
    public function get_team_tasks_board($filters = [])
    {
        $statusFilter = $filters['status'] ?? [];
        if ($statusFilter !== null && $statusFilter !== '' && ! is_array($statusFilter)) {
            $statusFilter = [$statusFilter];
        }

        $statusFilter = array_map('intval', array_filter((array) $statusFilter, function ($status) {
            return $status !== '' && $status !== null;
        }));

        $followersSelect = '(SELECT GROUP_CONCAT(CONCAT(firstname, " ", lastname) ORDER BY ' . db_prefix() . 'task_followers.id ASC SEPARATOR ",")
            FROM ' . db_prefix() . 'task_followers
            JOIN ' . db_prefix() . 'staff tf ON tf.staffid = ' . db_prefix() . 'task_followers.staffid
            WHERE taskid=' . db_prefix() . 'tasks.id) as follower_full_names';

        $this->db->select('staff.staffid, staff.firstname, staff.lastname, staff.profile_image, ' . db_prefix() . 'tasks.id as task_id, ' . db_prefix() . 'tasks.name as task_name, ' . db_prefix() . 'tasks.status, ' . db_prefix() . 'tasks.priority, ' . db_prefix() . 'tasks.startdate, ' . db_prefix() . 'tasks.duedate, ' . db_prefix() . 'tasks.rel_type, ' . db_prefix() . 'tasks.rel_id, ' . db_prefix() . 'tasks.kanban_order, ' . db_prefix() . 'tasks.dateadded');
        $this->db->select(tasks_rel_name_select_query() . ' as rel_name', false);
        $this->db->select(get_sql_select_task_total_checklist_items(), false);
        $this->db->select(get_sql_select_task_total_finished_checklist_items(), false);
        $this->db->select(get_sql_select_task_asignees_full_names() . ' as assignees', false);
        $this->db->select(get_sql_select_task_assignees_ids() . ' as assignee_ids', false);
        $this->db->select($followersSelect, false);

        $this->db->from(db_prefix() . 'task_assigned ta');
        $this->db->join(db_prefix() . 'tasks', db_prefix() . 'tasks.id = ta.taskid', 'inner');
        $this->db->join(db_prefix() . 'staff staff', 'staff.staffid = ta.staffid', 'inner');
        $this->db->where('staff.active', 1);

        if ($this->db->field_exists('deleted', db_prefix() . 'tasks')) {
            $this->db->where(db_prefix() . 'tasks.deleted', 0);
        }

        if (! empty($statusFilter)) {
            $this->db->where_in(db_prefix() . 'tasks.status', $statusFilter);
        }

        if (! empty($filters['staff'])) {
            $staffIds = array_filter(array_map('intval', (array) $filters['staff']));
            if (! empty($staffIds)) {
                $this->db->where_in('staff.staffid', $staffIds);
            }
        }

        if (! empty($filters['rel_type'])) {
            $this->db->where_in(db_prefix() . 'tasks.rel_type', (array) $filters['rel_type']);
        }
        ...
        if (! empty($filters['search'])) {
            ...
            if ($search !== '') {
                ...
                $this->db->group_start();
                $this->db->like(db_prefix() . 'tasks.name', $search);
                $this->db->or_like(db_prefix() . 'tasks.description', $search);
                $this->db->or_like('staff.firstname', $search);
        ...
        switch ($sort) {
            case 'due_date':
                $this->db->order_by(db_prefix() . 'tasks.duedate IS NULL', '', false);
                $this->db->order_by(db_prefix() . 'tasks.duedate', 'ASC');
                break;
            case 'recent':
                $this->db->order_by(db_prefix() . 'tasks.dateadded', 'DESC');
                break;
            case 'alpha':
                $this->db->order_by('staff.firstname', 'ASC');
                $this->db->order_by('staff.lastname', 'ASC');
            ...
            default:
                ...
                $this->db->order_by(db_prefix() . 'tasks.kanban_order', 'ASC');
                $this->db->order_by(db_prefix() . 'tasks.id', 'DESC');
                break;
        }
        ...
        foreach ($results as $row) {
            ...
            $assigneeIds   = $row['assignee_ids'] ? explode(',', $row['assignee_ids']) : [];
            $assigneeNames = $row['assignees'] ? explode(',', $row['assignees']) : [];
            $assigneePairs = [];
            foreach ($assigneeIds as $index => $assigneeId) {
                $assigneePairs[] = [
                    'id'   => (int) $assigneeId,
                    'name' => trim($assigneeNames[$index] ?? ''),
                ];
            }

            $followers = [];
            if (! empty($row['follower_full_names'])) {
                $followers = array_map('trim', explode(',', $row['follower_full_names']));
            }
            ...
            $board[$staffId]['tasks'][] = [
                ...
                'total_checklist_items'         => (int) $row['total_checklist_items'],
                'total_finished_checklist_items'=> (int) $row['total_finished_checklist_items'],
                'assignees'                     => $assigneePairs,
                'assignees_ids'                 => $row['assignee_ids'],
                'assignees_names'               => $row['assignees'],
                'followers'                     => $followers,
            ];
        }
    ...
    public function get_my_tasks_status_board($staff_id, $filters = [])
    {
        $statusFilter = $filters['status'] ?? [];
        if ($statusFilter !== null && $statusFilter !== '' && ! is_array($statusFilter)) {
            $statusFilter = [$statusFilter];
        ...
        $this->db->select('' . db_prefix() . 'tasks.status as task_status, ' . db_prefix() . 'tasks.id as task_id, ' . db_prefix() . 'tasks.name as task_name, ' . db_prefix() . 'tasks.status, ' . db_prefix() . 'tasks.priority, ' . db_prefix() . 'tasks.startdate, ' . db_prefix() . 'tasks.duedate, ' . db_prefix() . 'tasks.rel_type, ' . db_prefix() . 'tasks.rel_id, ' . db_prefix() . 'tasks.kanban_order, ' . db_prefix() . 'tasks.dateadded');
        $this->db->select(tasks_rel_name_select_query() . ' as rel_name', false);
        $this->db->select(get_sql_select_task_total_checklist_items(), false);
        $this->db->select(get_sql_select_task_total_finished_checklist_items(), false);
        $this->db->select(get_sql_select_task_asignees_full_names() . ' as assignees', false);
        ...
        // Include both assigned and followed tasks
        $this->db->from(db_prefix() . 'tasks');
        $this->db->join('(
            SELECT taskid, staffid FROM ' . db_prefix() . 'task_assigned
            UNION
            SELECT taskid, staffid FROM ' . db_prefix() . 'task_followers
        ) user_tasks', 'user_tasks.taskid = ' . db_prefix() . 'tasks.id', 'inner');
        $this->db->where('user_tasks.staffid', $staff_id);
    ...
    public function get($id, $where = [])
    {
        ...
        $this->db->where('id', $id);
        $this->db->where($where);
        // Soft delete: exclude deleted tasks
        if ($this->db->field_exists('deleted', db_prefix() . 'tasks')) {
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        $ticketReplyId = null;
        if (isset($data['ticket_reply_id']) && $data['ticket_reply_id'] !== '') {
            $ticketReplyId = (int) $data['ticket_reply_id'];
            unset($data['ticket_reply_id']); // reply id is used for prefill only, not a task column
        }

        if (isset($data['ticket_approval_flow_id'])) {
        ...
        if ($fromTicketId !== null) {
            // Temporarily disabled ticket follower functionality for testing
            $logContext = 'ticket ID ' . $fromTicketId;
            if ($ticketReplyId !== null) {
                $logContext .= ' (reply ' . $ticketReplyId . ')';
            }
            log_message('debug', 'Ticket-to-task conversion: Skipping automatic follower addition for ' . $logContext);
            /*
            try {
                $ticketFollowerIds = $this->tickets_model->get_ticket_staff_followers((int) $fromTicketId);
                if (is_array($ticketFollowerIds) && !empty($ticketFollowerIds)) {
                    if (!isset($followers) || !is_array($followers)) {
                        $followers = [];
                    }
                    $followers = array_values(array_unique(array_merge($followers, $ticketFollowerIds)));
                }
            } catch (Exception $e) {
                log_message('error', 'Error getting ticket followers for task creation: ' . $e->getMessage());
                // Continue with task creation without followers if there's an error
            }
            */
        }
    ...
    /**
     * Delete task and all connections (soft delete if column exists, hard delete otherwise)
    ...
    public function delete_task($id, $log_activity = true)
    {
        ...
        // Check if deleted column exists for soft delete
        if ($this->db->field_exists('deleted', db_prefix() . 'tasks')) {
            // Soft delete: set deleted = 1 instead of actually deleting
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'tasks', ['deleted' => 1]);
            if ($this->db->affected_rows() > 0) {
            ...
            // Fallback to hard delete if deleted column doesn't exist
            $this->db->where('id', $id);
     ...
     * @return bool
     */
    public function mark_as($status, $taskId)
    {
        ...
        } elseif ((int) $task->status === static::STATUS_COMPLETE) {
            // Clear completion timestamp when moving away from COMPLETE
            $data['datefinished'] = null;
        }

        $data = hooks()->apply_filters('before_task_mark_as', $data, $taskId, $task);
        ...
        log_activity('Task Status Updated [ID:' . $taskId . ', Status:' . $status . ']');

        $this->_send_task_responsible_users_notification(
            'not_task_status_changed',
            $taskId,
            false,
            '',
            serialize([
                $task->name,
                format_task_status($status, true, true),
            ])
        );
        ...
        if ($task->rel_type == 'project') {
            $activityKey = '';
            if ($status === static::STATUS_COMPLETE) {
                $activityKey = 'project_activity_task_marked_complete';
            } elseif ((int) $task->status === static::STATUS_COMPLETE && $status !== static::STATUS_COMPLETE) {
                $activityKey = 'project_activity_task_unmarked_complete';
            }

            if ($activityKey !== '') {
                $this->projects_model->log_activity($task->rel_id, $activityKey, $task->name, $task->visible_to_client);
            }
        }

        hooks()->do_action('after_task_status_change', [
            'task_id'    => $taskId,
    ...
    /**
     * Convenience wrapper used when reverting the completion state.
     *
     * @param  int $taskId
     * @return bool
     */
    public function unmark_complete($taskId)
    {
        return $this->mark_as(static::STATUS_NOT_STARTED, $taskId);
    }
