...
class Staff_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        $select_str = db_prefix().'staff.*,CONCAT(firstname,\' \',lastname) as full_name';

        if ($custom_field) {
            $select_str .= ',(SELECT '.db_prefix().'customfieldsvalues.value FROM '.db_prefix().'customfieldsvalues WHERE '.db_prefix().'customfieldsvalues.relid='.db_prefix().'staff.staffid AND '.db_prefix().'customfieldsvalues.fieldid='.$custom_field->id.') as staff_emp_code';
        }

        if ($division_field) {
            $select_str .= ',(SELECT '.db_prefix().'customfieldsvalues.value FROM '.db_prefix().'customfieldsvalues WHERE '.db_prefix().'customfieldsvalues.relid='.db_prefix().'staff.staffid AND '.db_prefix().'customfieldsvalues.fieldid='.$division_field->id.') as staff_division';
        }
    ...
    public function add($data)
    {
        ...
        // Remove UI-only department helper fields
        if (isset($data['member_parent_department'])) {
            unset($data['member_parent_department']);
        }
        if (isset($data['member_sub_department'])) {
            unset($data['member_sub_department']);
        }
    ...
    public function update_permissions($permissions, $id)
    {
        ...
        // Clear the permission cache for this staff member
