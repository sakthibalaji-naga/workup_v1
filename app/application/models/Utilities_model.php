...
class Utilities_model extends App_Model
{
    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        $has_permission_tickets = staff_can('view', 'tickets') || staff_can('view_own', 'tickets');

        // Get current user's departments for team tickets
        $this->db->select('departmentid');
        $this->db->from(db_prefix() . 'staff_departments');
        $this->db->where('staffid', get_staff_user_id());
        $user_departments = $this->db->get()->result_array();
        $user_department_ids = array_column($user_departments, 'departmentid');
        $team_member_ids = get_staff_subordinate_ids(null, true, false);

        // My Tickets
        if (!$client_data && !$ff || (!$client_data && $ff && array_key_exists('my_tickets', $filters))) {
            $ticketsTable = db_prefix() . 'tickets';

            // Response time tickets
            $this->db->select("ticketid, subject, $ticketsTable.date, approx_response_time, approx_resolution_time, company");
            $this->db->from($ticketsTable);
            $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid = ' . $ticketsTable . '.userid', 'left');
            $this->db->where('status !=', 5); // Exclude closed-resolved tickets
            $this->db->where('assigned', get_staff_user_id());
            $this->db->where('(approx_response_time BETWEEN "' . $start . '" AND "' . $end . '" AND approx_response_time IS NOT NULL)');

            $tickets_response = $this->db->get()->result_array();

            foreach ($tickets_response as $ticket) {
                $_ticket['id'] = $ticket['ticketid'];
                $_ticket['date'] = date('Y-m-d', strtotime($ticket['approx_response_time']));
                $subject = mb_substr($ticket['subject'], 0, 60) . '...';
                $_ticket['title'] = '🎫 ' . $subject;
                $current_date = date('Y-m-d');
                $response_date = date('Y-m-d', strtotime($ticket['approx_response_time']));
                $_ticket['color'] = ($current_date <= $response_date) ? '#28a745' : '#dc3545'; // Green if not past due, Red if past due
                $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                $_ticket['_tooltip'] = _l('my_tickets') . ' - ' . _l('response_time') . ' SLA: ' . $subject;
                if (isset($ticket['company']) && !empty($ticket['company'])) {
                    $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                }
                array_push($data, $_ticket);
            }

            // Reset query for resolution time tickets
            $this->db->reset_query();
            ...
            $this->db->where('status !=', 5); // Exclude closed-resolved tickets
            $this->db->where('assigned', get_staff_user_id());
            $this->db->where('(approx_resolution_time BETWEEN "' . $start . '" AND "' . $end . '" AND approx_resolution_time IS NOT NULL)');

            $tickets_resolution = $this->db->get()->result_array();

            foreach ($tickets_resolution as $ticket) {
                $_ticket['id'] = $ticket['ticketid'];
                $_ticket['date'] = date('Y-m-d', strtotime($ticket['approx_resolution_time']));
                $subject = mb_substr($ticket['subject'], 0, 60) . '...';
                $_ticket['title'] = '⏱️ ' . $subject;
                $current_date = date('Y-m-d');
                $resolution_date = date('Y-m-d', strtotime($ticket['approx_resolution_time']));
                $_ticket['color'] = ($current_date <= $resolution_date) ? '#28a745' : '#dc3545'; // Green if not past due, Red if past due
                $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                $_ticket['_tooltip'] = _l('my_tickets') . ' - ' . _l('resolution_time') . ' SLA: ' . $subject;
                if (isset($ticket['company']) && !empty($ticket['company'])) {
                    $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                }
                array_push($data, $_ticket);
        ...
        // My Team Tickets
        if (!$client_data && !$ff || (!$client_data && $ff && array_key_exists('my_team_tickets', $filters))) {
            $ticketsTable = db_prefix() . 'tickets';

            // Response time team tickets
            $this->db->select("ticketid, subject, $ticketsTable.date, approx_response_time, approx_resolution_time, company");
            $this->db->from($ticketsTable);
            $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid = ' . $ticketsTable . '.userid', 'left');
            $this->db->where('status !=', 5); // Exclude closed-resolved tickets
            if (!empty($user_department_ids)) {
                $this->db->where_in('department', $user_department_ids);
            }
            $this->db->where('(approx_response_time BETWEEN "' . $start . '" AND "' . $end . '" AND approx_response_time IS NOT NULL)');
            ...
            foreach ($tickets_response as $ticket) {
                ...
                $_ticket['color'] = ($current_date <= $response_date) ? '#28a745' : '#dc3545'; // Green if not past due, Red if past due
                $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                $_ticket['_tooltip'] = _l('my_team_tickets') . ' - ' . _l('response_time') . ' SLA: ' . $subject;
                if (isset($ticket['company']) && !empty($ticket['company'])) {
                    $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                }
                array_push($data, $_ticket);
            ...
            // Reset query for team resolution time tickets
            $this->db->reset_query();
            ...
            foreach ($tickets_resolution as $ticket) {
                ...
                $_ticket['color'] = ($current_date <= $resolution_date) ? '#28a745' : '#dc3545'; // Green if not past due, Red if past due
                $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                $_ticket['_tooltip'] = _l('my_team_tickets') . ' - ' . _l('resolution_time') . ' SLA: ' . $subject;
                if (isset($ticket['company']) && !empty($ticket['company'])) {
                    $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                }
                array_push($data, $_ticket);
        ...
        // My Team Creation Tickets
        if (!$client_data && (!$ff || ($ff && array_key_exists('my_team_creation_tickets', $filters)))) {
            $teamMemberIds = array_filter(array_map('intval', is_array($team_member_ids) ? $team_member_ids : []));
            if (!empty($teamMemberIds)) {
                $ticketsTable = db_prefix() . 'tickets';

                // Response time for team-created tickets
                $this->db->select("ticketid, subject, $ticketsTable.date, approx_response_time, approx_resolution_time, company");
                $this->db->from($ticketsTable);
                $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid = ' . $ticketsTable . '.userid', 'left');
                $this->db->where('status !=', 5);
                $this->db->where_in('admin', $teamMemberIds);
                $this->db->where('(approx_response_time BETWEEN "' . $start . '" AND "' . $end . '" AND approx_response_time IS NOT NULL)');

                $teamCreatedResponse = $this->db->get()->result_array();

                foreach ($teamCreatedResponse as $ticket) {
                    $_ticket['id'] = $ticket['ticketid'];
                    $_ticket['date'] = date('Y-m-d', strtotime($ticket['approx_response_time']));
                    $subject = mb_substr($ticket['subject'], 0, 60) . '...';
                    $_ticket['title'] = 'Team Created ' . $subject;
                    $current_date = date('Y-m-d');
                    $response_date = date('Y-m-d', strtotime($ticket['approx_response_time']));
                    $_ticket['color'] = ($current_date <= $response_date) ? '#28a745' : '#dc3545';
                    $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                    $_ticket['_tooltip'] = _l('my_team_creation_tickets') . ' - ' . _l('response_time') . ' SLA: ' . $subject;
                    if (isset($ticket['company']) && !empty($ticket['company'])) {
                        $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                    }
                    array_push($data, $_ticket);
                ...
                $teamCreatedResolution = $this->db->get()->result_array();

                foreach ($teamCreatedResolution as $ticket) {
                    $_ticket['id'] = $ticket['ticketid'];
                    $_ticket['date'] = date('Y-m-d', strtotime($ticket['approx_resolution_time']));
                    $subject = mb_substr($ticket['subject'], 0, 60) . '...';
                    $_ticket['title'] = 'Team Created Res ' . $subject;
                    $current_date = date('Y-m-d');
                    $resolution_date = date('Y-m-d', strtotime($ticket['approx_resolution_time']));
                    $_ticket['color'] = ($current_date <= $resolution_date) ? '#28a745' : '#dc3545';
                    $_ticket['url'] = admin_url('tickets/ticket/' . $ticket['ticketid']);
                    $_ticket['_tooltip'] = _l('my_team_creation_tickets') . ' - ' . _l('resolution_time') . ' SLA: ' . $subject;
                    if (isset($ticket['company']) && !empty($ticket['company'])) {
                        $_ticket['_tooltip'] .= ' (' . $ticket['company'] . ')';
                    }
                    array_push($data, $_ticket);
