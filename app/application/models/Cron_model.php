...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $this->process_expired_ticket_reassignments();
    ...
    private function process_expired_ticket_reassignments()
    {
        $time_limit_hours = get_option('ticket_reassignment_time_limit');

        if (!$time_limit_hours || $time_limit_hours == 0) {
            return;
        }

        // Get pending ticket reassignments that have exceeded the time limit
        $this->db->select('tra.id as reassignment_id, t.ticketid, t.userid, t.assigned, t.subject, tra.reason, tra.created_at, tra.from_staff_id, tra.to_staff_id');
        $this->db->from(db_prefix() . 'ticket_reassignments tra');
        $this->db->join(db_prefix() . 'tickets t', 't.ticketid = tra.ticket_id');
        $this->db->where('tra.status', 'pending');
        $this->db->where('tra.created_at <=', date('Y-m-d H:i:s', strtotime('-' . $time_limit_hours . ' hours')));
        $expired_reassignments = $this->db->get()->result_array();

        if (empty($expired_reassignments)) {
            return;
        }
        ...
        foreach ($expired_reassignments as $reassignment) {
            // Mark the reassignment as expired
            $this->db->where('id', $reassignment['reassignment_id']);
            $this->db->update(db_prefix() . 'ticket_reassignments', [
                'status' => 'expired',
                'processed_at' => date('Y-m-d H:i:s')
            ]);

            // Revert ticket back to original assignee
            $original_assignee = $reassignment['from_staff_id'];
            if ($reassignment['assigned'] != $original_assignee) {
                $this->db->where('ticketid', $reassignment['ticketid']);
                $this->db->update(db_prefix() . 'tickets', [
                ...
                if ($this->db->affected_rows() > 0) {
                    // Log activity
                    $this->tickets_model->log_ticket_activity($reassignment['ticketid'], 'Ticket reassignment request expired and reverted to original assignee');

                    // Notify relevant staff
                    $staff_to_notify = [];

                    // Notify the original assignee
                    if ($reassignment['from_staff_id'] && !in_array($reassignment['from_staff_id'], $notified_staff)) {
                        $staff_to_notify[] = $reassignment['from_staff_id'];
                        $notified_staff[] = $reassignment['from_staff_id'];
                    }

                    // Notify the target assignee who didn't respond
                    if ($reassignment['to_staff_id'] && !in_array($reassignment['to_staff_id'], $notified_staff)) {
                        $staff_to_notify[] = $reassignment['to_staff_id'];
                        $notified_staff[] = $reassignment['to_staff_id'];
                    }

                    // Notify staff with admin access or managers
                    $admin_staff = $this->staff_model->get('', ['active' => 1]);
                    foreach ($admin_staff as $admin) {
                        if (is_admin($admin['staffid'])) {
                            $staff_to_notify[] = $admin['staffid'];
                        }
                    }
                    ...
                    foreach ($staff_to_notify as $staff_id) {
                        ...
                        if ($staff_info && !in_array($staff_id, $notified_staff)) {
                            add_notification([
                                'description' => 'Ticket reassignment request expired - automatic reversion',
                                'touserid' => $staff_id,
                                'fromcompany' => true,
                                'fromuserid' => 0,
                                'link' => 'admin/tickets/ticket/' . $reassignment['ticketid'],
                                'additional_data' => serialize([
                                    'ticket_subject' => $reassignment['subject'],
                                    'reassignment_rreason' => $reassignment['reason']
                                ])
                            ]);

                            $notified_staff[] = $staff_id;
                        }
                    }
                }
            }
        ...
        // Trigger notifications if any staff were notified
        if (!empty($notified_staff)) {
            pusher_trigger_notification(array_unique($notified_staff));
        }
    }
