...
class Misc_model extends App_Model
{
    ...
    public function _search_tickets($q, $limit = 0, $filter_by_user_involvement = false)
    {
        $result = [
            'result'         => [],
            'type'           => 'tickets',
        ...
        if (is_staff_member() || (!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1)) {
            ...
            // Add user involvement filtering for task creation context
            if ($filter_by_user_involvement) {
                $user_id = get_staff_user_id();
                $involvement_condition = '(' . db_prefix() . 'tickets.addedfrom = ' . $user_id . ' OR ' . db_prefix() . 'tickets.assigned = ' . $user_id;
                $involvement_condition .= ' OR EXISTS (SELECT 1 FROM ' . db_prefix() . 'ticket_handlers WHERE ' . db_prefix() . 'ticket_handlers.ticket_id = ' . db_prefix() . 'tickets.ticketid AND ' . db_prefix() . 'ticket_handlers.staffid = ' . $user_id . '))';

                if ($where != '') {
                    $where .= ' AND ' . $involvement_condition;
                } else {
                    $where = $involvement_condition;
                }
            }
