...
class Reports_model extends App_Model
{
    ...
    /**
     * Get ticket lifecycle report data
     * @return array
     */
    public function get_ticket_lifecycle_report($from_date = null, $to_date = null, $department_filter = [], $status_filter = [], $division_filter = [])
    {
        ...
        // Date filter
        if ($from_date && $to_date) {
            $where_conditions[] = "t.date BETWEEN '" . $this->db->escape_str($from_date) . "' AND '" . $this->db->escape_str($to_date) . "'";
        } elseif ($from_date) {
            $where_conditions[] = "t.date >= '" . $this->db->escape_str($from_date) . "'";
        } elseif ($to_date) {
            $where_conditions[] = "t.date <= '" . $this->db->escape_str($to_date) . "'";
        }

        // Department filter
        if (!empty($department_filter)) {
            $deptIds = array_map('intval', $department_filter);
            $deptIds = array_filter($deptIds);
            if (!empty($deptIds)) {
                $where_conditions[] = "t.department IN (" . implode(',', $deptIds) . ")";
            }
        }

        // Status filter
        if (!empty($status_filter)) {
            $statusIds = array_map('intval', $status_filter);
            $statusIds = array_filter($statusIds);
            if (!empty($statusIds)) {
                $where_conditions[] = "t.status IN (" . implode(',', $statusIds) . ")";
            }
        }

        // Division filter
        if (!empty($division_filter)) {
            $divisionIds = array_map('intval', $division_filter);
            $divisionIds = array_filter($divisionIds);
            if (!empty($divisionIds)) {
                $where_conditions[] = "t.divisionid IN (" . implode(',', $divisionIds) . ")";
            }
        }
        ...
        $sql = "
            SELECT
                ...
                t.status,
                t.admin as created_by_staff_id,
                t.assigned as assigned_to_staff_id,
                t.department,
                t.service,
                t.divisionid,
                ...
                -- Assignee info
                assignee.firstname as assigned_to_firstname,
                assignee.lastname as assigned_to_lastname,
                ...
                -- First reply date (staff reply)
                (SELECT MIN(tr.date)
                 FROM " . db_prefix() . "ticket_replies tr
                 WHERE tr.ticketid = t.ticketid AND tr.admin IS NOT NULL
                 LIMIT 1) as first_staff_reply_date,

                -- Last reply date
                (SELECT MAX(tr.date)
                 FROM " . db_prefix() . "ticket_replies tr
                 WHERE tr.ticketid = t.ticketid
                 LIMIT 1) as last_reply_date,

                -- Reopen count from logs
                (SELECT COUNT(*) FROM " . db_prefix() . "ticket_logs tl
                 WHERE tl.ticketid = t.ticketid AND tl.log_type = 'status_change'
                 AND tl.log_details LIKE '%reopened%') as reopen_count,

                -- Fixed date (when status changed to resolved/closed)
                (SELECT tl.timestamp
                 FROM " . db_prefix() . "ticket_logs tl
                 WHERE tl.ticketid = t.ticketid AND tl.log_type = 'status_change'
                 AND tl.log_details LIKE '%\"new_status\":\"%Close%\"%'
                 ORDER BY tl.timestamp DESC
                 LIMIT 1) as fixed_date,
            ...
            LEFT JOIN " . db_prefix() . "staff creator ON creator.staffid = t.admin
            LEFT JOIN " . db_prefix() . "staff assignee ON assignee.staffid = t.assigned
            LEFT JOIN " . db_prefix() . "departments dept ON dept.departmentid = t.department
            LEFT JOIN " . db_prefix() . "divisions divs ON divs.divisionid = t.divisionid
            LEFT JOIN " . db_prefix() . "services serv ON serv.serviceid = t.service
            LEFT JOIN " . db_prefix() . "tickets_priorities pri ON pri.priorityid = t.priority
            LEFT JOIN " . db_prefix() . "tickets_status st ON st.ticketstatusid = t.status

            {$where_clause}

            ORDER BY t.date DESC, t.ticketid DESC
        ";

        $tickets = $this->db->query($sql)->result_array();

        // Process tickets to calculate lifespan and get handlers
        foreach ($tickets as &$ticket) {
            // Calculate lifespan
            $start_date = strtotime($ticket['created_date']);
            $end_date = $ticket['closed_date'] ? strtotime($ticket['closed_date']) : time();
            $lifespan_seconds = $end_date - $start_date;

            $ticket['lifespan_days'] = round($lifespan_seconds / (60 * 60 * 24), 1);
            $ticket['lifespan_hours'] = round($lifespan_seconds / (60 * 60), 1);

            // Get ticket handlers
            $handlers = $this->db->select('staffid')
                ->from(db_prefix() . 'ticket_handlers')
            ...
            foreach ($handlers as $handler) {
                $staff = $this->staff_model->get($handler['staffid']);
                if ($staff) {
                    $handlers_info[] = $staff->firstname . ' ' . $staff->lastname;
                }
            }
            $ticket['handlers_list'] = implode(', ', $handlers_info);

            // Get formatted dates
            $ticket['created_date_formatted'] = _d($ticket['created_date']);
            $ticket['first_staff_reply_date_formatted'] = $ticket['first_staff_reply_date'] ? _d($ticket['first_staff_reply_date']) : '';
            $ticket['last_reply_date_formatted'] = $ticket['last_reply_date'] ? _d($ticket['last_reply_date']) : '';
            $ticket['fixed_date_formatted'] = $ticket['fixed_date'] ? _d($ticket['fixed_date']) : '';
            $ticket['closed_date_formatted'] = $ticket['closed_date'] ? _d($ticket['closed_date']) : '';

            // Calculate response time (first reply - created)
            if ($ticket['first_staff_reply_date']) {
                $response_time = strtotime($ticket['first_staff_reply_date']) - strtotime($ticket['created_date']);
                $ticket['response_time_hours'] = round($response_time / (60 * 60), 1);
                $ticket['response_time_days'] = round($response_time / (60 * 60 * 24), 1);
            } else {
                $ticket['response_time_hours'] = null;
                $ticket['response_time_days'] = null;
            }
        }
