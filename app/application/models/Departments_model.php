...
class Departments_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['divisions'])) {
            $divisions = array_filter(array_map('intval', (array) $data['divisions']));
            unset($data['divisions']);
        }
        // Avoid DB errors if schema not migrated yet
        if (isset($data['responsible_staff']) && !$this->db->field_exists('responsible_staff', db_prefix() . 'departments')) {
            unset($data['responsible_staff']);
        }
    ...
    public function update($data, $id)
    {
        ...
        // Even if no changes in department table, still sync divisions if provided
        if (is_array($divisions)) {
            // Fetch existing
            $existing = $this->db->select('divisionid')->where('departmentid', $id)->get(db_prefix().'department_divisions')->result_array();
            $existingIds = array_map(function($r){return (int)$r['divisionid'];}, $existing);
            $toInsert = array_diff($divisions, $existingIds);
            $toDelete = array_diff($existingIds, $divisions);
            if (!empty($toDelete)) {
                $this->db->where('departmentid', $id);
                $this->db->where_in('divisionid', $toDelete);
                $this->db->delete(db_prefix().'department_divisions');
                $divisionsChanged = true;
            }
            foreach ($toInsert as $divisionid) {
                $this->db->insert(db_prefix().'department_divisions', [
                    'departmentid' => $id,
                    'divisionid'   => $divisionid,
                ]);
        ...
        return $baseUpdated || $divisionsChanged;
    }
