...
function get_relation_data($type, $rel_id = '', $extra = [])
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $CI->load->model('approval_flow_model');
        if ($rel_id != '') {
            $data = $CI->approval_flow_model->get($rel_id);
        } else {
            $CI->db->select('id, name, created_by, status');
            $CI->db->from(db_prefix() . 'approval_flows');
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if (is_array($relation)) {
            ...
            $name = $relation['ticket_number'] ?? ('#' . $relation['ticketid']);
            $name .= ' - ' . $relation['subject'];
        } else {
            $id   = $relation->ticketid;
            $name = $relation->ticket_number ?? ('#' . $relation->ticketid);
        ...
        $link = admin_url('approval_flow/approval_flow/' . $id);
    }
