...
function handle_ticket_attachments($ticketid, $index_name = 'attachments')
{
    ...
    if (isset($_FILES[$index_name])) {
        ...
        $max_attachments = (int) get_option('maximum_allowed_ticket_attachments');
        $total_files = count($_FILES[$index_name]['name']);

        // Check if total files exceed the limit
        if ($max_attachments > 0 && $total_files > $max_attachments) {
            // Only process the first $max_attachments files
            $files_to_process = array_slice($_FILES[$index_name]['name'], 0, $max_attachments);
            $end_index = $max_attachments;
        } else {
        ...
        for ($i = 0; $i < $end_index; $i++) {
            ...
            // Get the temp file path
            $tmpFilePath = $_FILES[$index_name]['tmp_name'][$i];
            $fileName = $_FILES[$index_name]['name'][$i];

            // Debug logging for each file
            log_message('debug', 'Processing ticket attachment ' . ($i + 1) . '/' . $end_index . ': ' . $fileName . ' (tmp: ' . $tmpFilePath . ')');

            // Make sure we have a filepath
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                // Check if extension is allowed using the same function as other uploads
                if (!_upload_extension_allowed($fileName)) {
                    log_message('debug', 'File extension not allowed: ' . $fileName);
                    continue;
                }
                _maybe_create_upload_path($path);
                ...
                log_message('debug', 'Moving file: ' . $tmpFilePath . ' -> ' . $newFilePath);
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    log_message('error', 'Failed to move file: ' . $tmpFilePath . ' -> ' . $newFilePath);
                }
            } else {
                log_message('debug', 'Empty temp file path for: ' . $fileName);
            }
        }
    }
...
{
    // Collect all files from multiple inputs like attachments[0], attachments[1], etc.
    $collected_files = [
        'name' => [],
        'type' => [],
        'tmp_name' => [],
    ...
    // Loop through all possible attachment inputs
    foreach ($_FILES as $key => $file) {
        if (strpos($key, $index_name) === 0) {
            // Handle both regular arrays and nested arrays from multiple inputs
            if (isset($file['name'])) {
                if (is_array($file['name'])) {
                    // Multiple files in one input (e.g., attachments[0] with multiple attribute)
                    foreach ($file['name'] as $file_key => $file_name) {
                        if (!empty($file_name) && !empty($file['tmp_name'][$file_key])) {
                            $collected_files['name'][] = $file_name;
                            $collected_files['type'][] = $file['type'][$file_key];
                            $collected_files['tmp_name'][] = $file['tmp_name'][$file_key];
                            $collected_files['error'][] = $file['error'][$file_key];
                            $collected_files['size'][] = $file['size'][$file_key];
                        }
                    }
                } else {
                    // Single file in an input (e.g., attachments[1], attachments[2], etc.)
                    if (!empty($file['name']) && !empty($file['tmp_name'])) {
                        $collected_files['name'][] = $file['name'];
                        $collected_files['type'][] = $file['type'];
                        $collected_files['tmp_name'][] = $file['tmp_name'];
                        $collected_files['error'][] = $file['error'];
                        $collected_files['size'][] = $file['size'];
                    }
                }
            }
        }
    ...
    // Replace the original $_FILES array with the collected files
    $_FILES[$index_name] = $collected_files;

    // Debug logging to see what we collected
    if (!empty($collected_files['name'])) {
        log_message('debug', 'File attachments collected for ' . $index_name . ': ' . json_encode([
            'count' => count($collected_files['name']),
            'names' => $collected_files['name'],
            'tmp_names' => $collected_files['tmp_name']
        ]));
    }
}
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        break;
        case 'ticket_sla_attachments':
            $path = FCPATH . 'uploads/ticket_sla_attachments' . '/';
