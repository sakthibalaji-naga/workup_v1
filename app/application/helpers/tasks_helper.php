...
{
    $CI = &get_instance();
    if (!property_exists($CI, 'tasks_model')) {
...
function format_members_by_ids_and_names($ids, $names, $size = 'md', $maxVisible = null)
{
    ...
    $assigneeNames = explode(',', (string) $names);
    $assigneeIds   = explode(',', (string) $ids);

    $members = [];
    foreach ($assigneeIds as $index => $assignee_id) {
        $assignee_id = trim($assignee_id);
        $assigned    = trim($assigneeNames[$index] ?? '');
        if ($assignee_id === '' || $assigned === '') {
            continue;
        }
        $members[] = [
            'id'   => $assignee_id,
    ...
    $overflowMembers = [];
    if ($maxVisible !== null && $maxVisible > 0 && count($members) > $maxVisible) {
        $visibleMembers  = array_slice($members, 0, $maxVisible);
        $overflowMembers = array_slice($members, $maxVisible);
    }

    foreach ($visibleMembers as $member) {
        $outputAssignees .= '<a href="' . admin_url('profile/' . $member['id']) . '">' .
            staff_profile_image($member['id'], [
                ($size == 'md' ? 'tw-h-7 tw-w-7' : 'tw-h-5 tw-w-5') . ' tw-inline-block tw-rounded-full tw-ring-2 tw-ring-white',
            ], 'small', [
                'data-toggle' => 'tooltip',
                'data-title'  => e($member['name']),
            ]) . '</a>';
        $exportAssignees .= e($member['name']) . ', ';
    }

    if (! empty($overflowMembers)) {
        $overflowHtml = '<div class="tw-flex tw-flex-col tw-gap-1">';
        foreach ($overflowMembers as $member) {
            $overflowHtml .= '<div class="tw-flex tw-items-center tw-gap-2">';
            $overflowHtml .= staff_profile_image($member['id'], [
                'tw-h-6 tw-w-6 tw-rounded-full tw-ring-2 tw-ring-white',
            ], 'small');
            $overflowHtml .= '<span class="tw-text-xs tw-text-slate-700">' . e($member['name']) . '</span>';
            $overflowHtml .= '</div>';
        }
        $overflowHtml .= '</div>';

        $outputAssignees .= '<span class="tw-inline-flex tw-h-7 tw-w-7 tw-items-center tw-justify-center tw-rounded-full tw-bg-white tw-text-[11px] tw-font-semibold tw-border tw-border-slate-200 task-assignee-overflow-trigger" data-toggle="popover" data-trigger="hover" data-html="true" data-container="body" data-placement="top" data-content="' . htmlspecialchars($overflowHtml, ENT_QUOTES, 'UTF-8') . '">+' . count($overflowMembers) . '</span>';
    }
 ...
 * Also includes tasks where the user is involved in task approvals
 * @return string
 */
function get_tasks_where_string($table = true)
{
    ...
    // Include tasks where user is involved in task approvals
    $_tasks_where .= ' OR ' . db_prefix() . 'tasks.id IN (SELECT task_id FROM ' . db_prefix() . 'task_approvals WHERE staff_id = ' . get_staff_user_id() . ')';
