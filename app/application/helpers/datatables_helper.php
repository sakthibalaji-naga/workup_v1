...
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
{
    ...
    if ($CI->input->post('order')) {
        ...
        foreach ($CI->input->post('order') as $key => $val) {
            ...
            if ((in_array($sTable . '.' . $columnName, $nullColumnsAsLast)
                ...
                $sOrder .= 'ISNULL(' . $columnName . ') ASC, ' . $columnName . ' ' . $dir;
            } else {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        // Skip searching on literal values like '1' for bulk actions
        if (preg_match('/^[\'\"]*[^a-zA-Z_]\w*[\'\"]*/', $columnName) || $columnName == '1') {
            continue;
        }
    ...
    if (count($additionalSelect) > 0) {
        foreach ($additionalSelect as $searchAdditionalField) {
            ...
            // Skip searching on expressions that are not valid for LIKE
            if (!preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)*$/', $searchAdditionalField)) {
                continue;
            }

            if (stripos($searchAdditionalField, 'AVG(') === false && stripos($searchAdditionalField, 'SUM(') === false) {
                // Use index
    ...
    $filteredResult = $CI->db->query("\n        SELECT COUNT(*) as iFilteredTotal\n        FROM {$sTable}\n        " . $join . "\n        {$sWhere}\n        " . $where . "\n        {$sGroupBy}\n    ")->row();
    $iFilteredTotal = $filteredResult ? $filteredResult->iFilteredTotal : 0;
    ...
    $totalResult = $CI->db->query("SELECT COUNT(*) as iTotal from {$sTable} {$join} {$where}")->row();
    $iTotal = $totalResult ? $totalResult->iTotal : 0;
