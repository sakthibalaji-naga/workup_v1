...
function render_date_input($name, $label = '', $value = '', $input_attrs = [], $form_group_attr = [], $form_group_class = '', $input_class = '')
{
    ...
    if (!isset($input_attrs['readonly']) || $input_attrs['readonly'] !== true) {
...
function render_select($name, $options, $option_attrs = [], $label = '', $selected = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true)
{
    ...
    if (!is_array($options)) {
        log_message('error', 'render_select called with invalid options type: ' . gettype($options) . ' - value: ' . print_r($options, true));
        return '<div class="form-group"><p class="text-danger">Error: Invalid options format</p></div>';
    }
...
function render_select_with_input_group($name, $options, $option_attrs = [], $label = '', $selected = '', $input_group_contents = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true)
{
    // Ensure $options is an array
    if (!is_array($options)) {
        log_message('error', 'render_select_with_input_group called with invalid options type: ' . gettype($options) . ' - value: ' . print_r($options, true));
        return '<div class="form-group"><p class="text-danger">Error: Invalid options format</p></div>';
    }
...
if (!function_exists('render_form_builder_field')) {
    ...
    {
        // Ensure $field is an object, not a string or other type
        if (!is_object($field)) {
            log_message('error', 'render_form_builder_field called with invalid field type: ' . gettype($field) . ' - value: ' . print_r($field, true));
            return; // Exit gracefully instead of causing a fatal error
        }
