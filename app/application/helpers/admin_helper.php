...
function render_admin_js_variables()
{
    ...
    $notificationIcon = base_url('assets/images/user-placeholder.jpg');
    foreach (['company_logo_dark', 'company_logo'] as $logoOption) {
        $logoFilename = get_option($logoOption);
        if (!empty($logoFilename)) {
            $logoPath = FCPATH . 'uploads/company/' . $logoFilename;
            if (file_exists($logoPath)) {
                $notificationIcon = base_url('uploads/company/' . $logoFilename);
                break;
            }
        }
    }
    ...
    $options = [
        ...
        'notifications_default_icon'                  => $notificationIcon,
