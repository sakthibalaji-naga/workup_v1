...
function get_mail_template_path($class, &$params)
{
    ...
    if (isset($params[0]) && is_string($params[0]) && $params[0] !== '/') {
        ...
        if (is_dir(module_dir_path($moduleName))) {
            $module = $CI->app_modules->get($moduleName);

            if (is_array($module) && isset($module['activated']) && (int) $module['activated'] === 1) {
                $dir = module_libs_path($moduleName) . 'mails/';
                unset($params[0]);
                $params = array_values($params);
            }
        }
