...
if (!function_exists('get_staff_subordinate_ids')) {
    /**
     * Get active staff IDs that report to the given manager.
     *
     * @param  int|null $managerId
     * @param  bool     $includeIndirect
     * @param  bool     $includeManager
     * @return array<int>
     */
    function get_staff_subordinate_ids($managerId = null, $includeIndirect = true, $includeManager = false)
    {
        $CI = & get_instance();

        if ($managerId === null) {
            $managerId = get_staff_user_id();
        }
        ...
        static $hasReportingColumn = null;

        if ($hasReportingColumn === null) {
            $hasReportingColumn = $CI->db->field_exists('reporting_manager', db_prefix() . 'staff');
        }

        if (!$hasReportingColumn) {
            return [];
        }
        ...
        while (!empty($queue)) {
            ...
            $CI->db->where('reporting_manager', $current);
            ...
            foreach ($rows as $row) {
                ...
                if (!$includeManager && $staffId === $managerId) {
                    continue;
                }

                $seen[$staffId] = true;
                $subordinates[] = $staffId;
        ...
        if ($includeManager && !in_array($managerId, $subordinates, true)) {
