...
function app_init_admin_sidebar_menu_items()
{
    ...
    if (
        staff_can('view', 'approval_flow')
        || staff_can('create', 'approval_flow')
        || staff_can('edit', 'approval_flow')
        || staff_can('delete', 'approval_flow')
    ) {
        $CI->app_menu->add_sidebar_menu_item('approval_flow', [
            'name'     => _l('approval_flow'),
            'href'     => admin_url('approval_flow'),
    ...
    // Reports menu with per-staff access filtering (Report Access tab)
    $reportAccessMeta = [];
    if (!is_admin()) {
        $reportAccessMetaRaw = get_staff_meta(get_staff_user_id(), 'report_access');
        $reportAccessMeta    = is_string($reportAccessMetaRaw) ? json_decode($reportAccessMetaRaw, true) : [];
        if (!is_array($reportAccessMeta)) {
            $reportAccessMeta = [];
        }
    }

    $reportAllowed = function ($slug) use ($reportAccessMeta) {
        if (is_admin()) {
            return true;
        }
        $config = $reportAccessMeta[$slug] ?? [];
        if (!is_array($config)) {
            return false;
        }
        return !empty($config['global'])
            || !empty($config['own'])
            || (isset($config['division_ids']) && is_array($config['division_ids']) && count($config['division_ids']) > 0)
            || (isset($config['department_ids']) && is_array($config['department_ids']) && count($config['department_ids']) > 0);
    };

    $reportsChildren = [];

    if (staff_can('view-timesheets', 'reports') && $reportAllowed('timesheets')) {
        $reportsChildren[] = [
    ...
    if (staff_can('view',  'reports') && $reportAllowed('sales-reports')) {
        $reportsChildren[] = [
            'slug'     => 'sales-reports',
    ...
    if (staff_can('view',  'reports') && $reportAllowed('expenses-reports')) {
        $reportsChildren[] = [
    ...
    if (staff_can('view',  'reports') && $reportAllowed('expenses-vs-income-reports')) {
        $reportsChildren[] = [
    ...
    if (staff_can('view',  'reports') && $reportAllowed('leads-reports')) {
        $reportsChildren[] = [
            'slug'     => 'leads-reports',
    ...
    if (staff_can('view',  'reports') && $reportAllowed('knowledge-base-reports')) {
        $reportsChildren[] = [
    ...
    if (staff_can('view',  'reports') && $reportAllowed('tickets-lifecycle-reports')) {
        $reportsChildren[] = [
            'slug'     => 'tickets-lifecycle-reports',
            'name'     => _l('ticket_lifecycle_report'),
            'href'     => admin_url('reports/tickets_lifecycle'),
            'position' => 35,
            'badge'    => [],
        ];
    }
    ...
    if (!empty($reportsChildren)) {
        ...
        foreach ($reportsChildren as $child) {
            $CI->app_menu->add_sidebar_children_item('reports', $child);
        }
    }
